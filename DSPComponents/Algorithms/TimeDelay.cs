...
namespace DSPAlgorithms.Algorithms
{
    public class TimeDelay : Algorithm
    {
        ...
        {
            DirectCorrelation cor = new DirectCorrelation();
            cor.InputSignal1 = InputSignal1;
            cor.InputSignal2 = InputSignal2;
            cor.Run();

            float max = Math.Abs(cor.OutputNormalizedCorrelation[0]);
            int index = 0;
            for (int i = 0; i < InputSignal1.Samples.Count; i++)
            {
                if (cor.OutputNormalizedCorrelation[i] > max)
                    max = cor.OutputNormalizedCorrelation[i];
                index = i;
            }
